  ...
  return (
    <div className={clsx('mt-4', className)}>
        ...
        {visibleEvents.map((event, index) => {
          ...
          return (
            <li key={`${event.timestamp}-${index}`} className="relative">
              <span className="absolute -left-2 mt-1 h-3 w-3 rounded-full bg-[rgb(0,115,255)] shadow-depth-sm" />
      ...
      {sortedEvents.length > initialVisible && (
        <button
          type="button"
          className="mt-3 text-sm font-semibold text-primary hover:text-[rgba(0,115,255,0.85)]"
